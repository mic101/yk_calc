...
function get_youku_phone_address_by_id($id) {
    return yk_ck_id($id);
}
...
function yk_list_begin($vid)
{
    ...
    if (property_exists($fileids, '3gphd')) {
        $urlformat = "mp4";
        $format = '3gphd';
    } else if (property_exists($fileids, 'mp4') ){
        $urlformat = $format;
        $format = 'mp4';
    } else {
        $urlformat = $format;
...
';
    ...
    foreach ($segs as $k => $v) {
        ...
        $tvaddr = KUR . $sid . '_00/st/' . $urlformat . '/fileid/' . $fileid . '?K=' . $key . '&hd=0&myp=0&ts=';
